...
namespace klbotlib.Modules
{
    public class IMGPModule : Module
    {
        ...
        public override string Processor(Message msg, int status_code)
        {
            ...
            if (status_code == 2)
            {
                HostBot.ReplyPlainMessage(this, msg, "转换中...");
            ...
            switch (status_code)
            {
                case 3: //识别
                    ...
                    HostBot.ReplyPlainMessage(this, msg, "识别中...");
                    if (type == "landmark")
                    {
                ...
                case 5: //本地压缩
                    HostBot.ReplyPlainMessage(this, msg, $"正在下载图片...");
                    Bitmap bmp = img_helper.DownloadImage(ipmsg.UrlList[0], out int original_size);
                    MemoryStream ms = new MemoryStream();
                    HostBot.ReplyPlainMessage(this, msg, "本地压缩中...");
                    bmp.Save(ms, ImageFormat.Jpeg);
                    byte[] bin = ms.ToArray();
                    string b64 = Convert.ToBase64String(bin);
                    HostBot.ReplyPlainMessage(this, msg, $"压缩完成。原始大小为{original_size.ToMemorySizeString(1)}，返图大小为{bin.Length.ToMemorySizeString(1)}");
                    return $@"\image:\base64:{b64}";
                case 6: //图像处理
