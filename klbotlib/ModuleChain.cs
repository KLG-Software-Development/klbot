...
public class ModuleChain : IEnumerable<Module>
{
    ...
    private readonly Dictionary<string, int> _moduleCountByName = [];
    ...
    //计算模块ID
    internal static string CalcModuleId(string module_name, int moduleIndex)
    {
        return moduleIndex == 0 ? module_name : $"{module_name}#{moduleIndex}";
    }
    //计算模块ID
    ...
    internal void AddModule(params Module[] modules)
    {
        ...
        {
            _indexById.Add(m.ModuleId, _modules.Count);      //添加ID到ID-索引字典
    ...
    public Module this[string moduleId]
    {
        get
        {
            return !_indexById.ContainsKey(moduleId)
                ? throw new ModuleMissingException($"在模块链条中不存在模块\"{moduleId}\"")
                : _modules[_indexById[moduleId]];
        }
        set
        {
